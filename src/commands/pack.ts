...
export async function main(): Promise<number> {
  ...
  if (project === null) {
    fail("You should't use `bld pack` in a workspace folder.")
  }
  ...
  const ignoreFileContent = await fs.readFile(
    path.resolve(workingDir, '.bldignore'),
    'utf-8'
  )
...
Please try to remove unneccessary files and/or reduce the files size, you can ignore file adding in .bldignore.`)
    }
