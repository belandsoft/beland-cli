...
export const help = () => `
  Usage: ${chalk.bold("bld workspace SUBCOMMAND [options]")}
  
  ${chalk.dim("Sub commands:")}
    
    init             Create a workspace looking for subfolder Beland projects.
...
async function init() {
  try {
    ...
    console.log(
      chalk.green(`\nSuccess! Run 'bld start' to preview your workspace.\n`)
    );
  } catch (err: any) {
...
async function addProject() {
  ...
  console.log(
    chalk.green(
      `\nSuccess! Run 'bld start' to preview your workspace and see the new project added.\n`
    )
  );
}
