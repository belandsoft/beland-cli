...
type BelandPackage = PackageJson<{ belandLibrary: string }>;
...
export async function getBelandDependencies(
  ...
  for (const dependency of Object.keys(dependencies)) {
    ...
    if (fs.pathExistsSync(modulePath)) {
      const pkgJson = await readJSON<BelandPackage>(modulePath);
      if (pkgJson.belandLibrary && pkgJson.name && pkgJson.version) {
