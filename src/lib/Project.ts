...
export class Project {
  ...
  async getSceneFile(): Promise<Scene> {
    ...
    try {
      ...
      fail(
        ...
        `Unable to read 'scene.json' file. Try initializing the project using 'bld init'.
  ...
  /**
   * Writes the `.bldignore` file to the provided directory path.
   * @param dir The target path where the file will be
   */
  async writeBldIgnore(): Promise<string> {
    const content = [
      '.*',
      'package.json',
      'package-lock.json',
    ...
    await fs.outputFile(
      path.join(this.projectWorkingDir, BLDIGNORE_FILE),
      content
    )
    return content
  }
   ...
   * All the paths added to the `.bldignore` file will be excluded from the results.
   * Windows directory separators are replaced for POSIX separators.
   * @param ignoreFile The contents of the .bldignore file
   */
  ...
  /**
   * Returns the the contents of the `.bldignore` file
   */
  async getBLDIgnore(): Promise<string | null> {
    let ignoreFile
