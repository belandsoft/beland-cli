...
export class Preview extends EventEmitter {
  ...
  constructor(public bld: Beland, watch: boolean) {
    super()
    this.watch = watch
  }
  ...
  async startServer(port: number) {
    ...
    if (this.watch) {
      for (const project of this.bld.workspace.getAllProjects()) {
        const ig = ignore().add((await project.getBLDIgnore())!)
    ...
    const artifactPath = fs.pathExistsSync(bldEcsPath) ? bldEcsPath : dclApiPath
    const unityPath = path.resolve(bldEcsPath, 'artifacts', 'unity')

    if (fs.existsSync(proxySetupPath)) {
      try {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const setupProxy = require(proxySetupPath)
        setupProxy(this.bld, this.app, express)
      } catch (err) {
        console.log(
    ...
    if (!fs.pathExistsSync(artifactPath)) {
      fail(
        ErrorType.PREVIEW_ERROR,
        `Couldn\'t find ${dclApiPath} or ${bldEcsPath}, please run: npm install`
      )
    }
    ...
    this.app.use('/contents/', express.static(this.bld.getWorkingDir()))

    this.app.get('/mappings', (req, res) => {
      glob(this.bld.getWorkingDir() + '/**/*', (err, _) => {
        if (err) {
          res.status(500)
          res.json(err)
          res.end()
          ...
          ret.contents = ret.contents.map(({ file, hash }: any) => ({
            file,
            hash: `contents/${hash}`
          }))
    ...
    this.app.get('/scene.json', (_, res) => {
      res.sendFile(path.join(this.bld.getWorkingDir(), 'scene.json'))
    })
...
function setDebugRunner(wss: WebSocket.Server) {
  wss.on('connection', (ws) => {
    if (ws.protocol === 'bld-scene') {
      const file = require.resolve('bld-node-runtime')
